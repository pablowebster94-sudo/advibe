...
export type PhotoStatus =
  ...
  | "edited"
  /**
   * The file was read and its metadata extracted, but no image data could be
   * decoded from it. Distinct from "error" on purpose: the photograph is still
   * in the project, its EXIF is still shown, and the reason is stated.
   */
  | "unreadable"
  | "error";
...
export interface PhotoRecord {
  ...
  /**
   * Pixel size of the stored proxy. Kept on the record so the develop view can
   * ask the decoder for the size it needs in one pass, instead of decoding the
   * proxy at full size and then resizing it.
   */
  proxyWidth?: number;
  proxyHeight?: number;
