...
import {getMetaBrowserData,track} from "@/components/enfoque/Tracking";

export default function Contacto(){
  const [form,setForm]=useState({name:"",phone:"",email:"",interest_type:"informacion_general",message:""});
  const [state,setState]=useState<"idle"|"sending"|"ok"|"error">("idle");
  async function submit(e:React.FormEvent){
    e.preventDefault(); setState("sending");
    try{
      const eventId=crypto.randomUUID();
      const meta=getMetaBrowserData();
      track("Lead",{event_id:eventId,content_type:form.interest_type});
      const r=await fetch("/api/enfoque/leads",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...form,channel:"formulario",event_id:eventId,...meta})});
      if(!r.ok)throw new Error();
      setState("ok"); setForm({name:"",phone:"",email:"",interest_type:"informacion_general",message:""});
  ...
  return <><Header/><main className="mx-auto max-w-6xl px-5 py-14 md:py-20"><div className="grid gap-12 lg:grid-cols-[1fr_520px] lg:items-start">
    ...
    <section className="rounded-[2rem] bg-black p-6 text-white md:p-8"><h2 className="text-2xl font-black">Solicitar información</h2><form onSubmit={submit} className="mt-6 space-y-4">
      ...
      {state==="ok"&&<p className="text-sm text-[#d9ff3f]">Solicitud recibida. Te contactaremos pronto.</p>}{state==="error"&&<p className="text-sm text-red-300">No pudimos registrar la solicitud. Intenta nuevamente.</p>}
