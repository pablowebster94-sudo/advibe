...
export const metadata: Metadata = {
  ...
  description:
    "Propuesta de contenido audiovisual, diseño gráfico y Meta Ads para Muebles Ideal.",
...
const pillars = [
  {
    ...
    text: "Videos pensados para redes sociales que muestran productos, materiales, detalles, promociones y beneficios de forma atractiva.",
  },
  {
    number: "02",
    title: "Producción audiovisual",
    text: "Una estética visual profesional para elevar la percepción de los productos y mostrar el showroom y la fabricación con fidelidad.",
  },
  {
    number: "03",
    title: "Publicidad dirigida",
    text: "Campañas de Meta Ads orientadas a conectar los contenidos y promociones con personas con mayor probabilidad de interés.",
  },
  {
    number: "04",
    title: "Diseño que acompaña",
    text: "Piezas gráficas para promociones, productos, campañas y comunicación comercial que complementan el contenido audiovisual.",
  },
];
    ...
    <main className="min-h-screen bg-[#f9f8f6] text-[#132a22] selection:bg-[#c1a162]/20">
      <section className="relative flex min-h-screen items-center overflow-hidden bg-[#132a22] px-6 py-24 text-[#f9f8f6] md:px-12">
        ...
        <div className="mx-auto flex w-full max-w-6xl flex-col justify-between gap-20">
          <div className="max-w-5xl">
            <p className="mb-10 text-xs font-medium uppercase tracking-[0.3em] text-[#c1a162]">
              Propuesta Comercial
            </p>
            <h1 className="font-serif text-5xl font-normal leading-[1.05] sm:text-6xl lg:text-8xl">
              ...
              <br />
              <span className="italic text-[#c1a162]">Muebles Ideal</span>
            </h1>
          ...
          <div>
            <p className="font-serif text-2xl sm:text-3xl">
              AdVibe Agencia <span className="mx-3 text-[#c1a162]">×</span> Muebles Ideal
            </p>
      ...
      <section className="px-6 py-24 md:px-12 md:py-32">
        <div className="mx-auto grid max-w-6xl gap-20 lg:grid-cols-2 lg:items-center lg:gap-28">
          <div>
            <h2 className="font-serif text-4xl font-normal leading-[1.08] sm:text-5xl lg:text-6xl">
              Una marca con historia.
              <br />
              <span className="italic">Una oportunidad para mostrarla mejor.</span>
            </h2>
            <div className="mt-10 space-y-6 text-lg font-light leading-relaxed text-slate-600">
              <p>
                Muebles Ideal cuenta con más de 50 años de experiencia en la fabricación de muebles y una presencia comercial consolidada.
              </p>
              <p>Hoy, el reto no es comenzar desde cero.</p>
              <p className="border-l-2 border-[#132a22] pl-5 font-medium text-[#132a22]">
                Es llevar todo ese valor a las plataformas digitales de una manera más atractiva, actual y estratégica.
              </p>
            </div>
          </div>
        ...
        <div className="mx-auto mt-20 max-w-6xl bg-[#132a22] p-10 text-[#f9f8f6] sm:p-14 lg:p-16">
          ...
          <p className="font-serif text-2xl leading-snug sm:text-4xl">
            Mostrar no solamente el mueble, sino lo que ese mueble representa: <span className="italic text-[#c1a162]">calidad, diseño, comodidad y un espacio para vivir.</span>
          </p>
        </div>
      </section>
      ...
      <section className="bg-white px-6 py-24 md:px-12 md:py-32">
        <div className="mx-auto max-w-6xl">
          <div className="mb-16 text-center md:mb-20">
            ...
            <p className="mt-4 text-xs font-medium uppercase tracking-[0.2em] text-slate-400">Los valores expresados son mensuales</p>
          </div>
            ...
            {plans.map((plan) => (
              <article
                ...
                className={`relative flex flex-col p-8 ${
                  plan.recommended
                    ? "border-2 border-[#132a22] bg-[#132a22] text-white shadow-2xl lg:-translate-y-4"
                    : "border border-slate-200 bg-[#f9f8f6]"
                }`}
              >
                {plan.recommended && (
                  <span className="absolute right-0 top-0 bg-[#c1a162] px-4 py-2 text-[10px] font-bold uppercase tracking-[0.18em] text-white">
                    Recomendado
                  </span>
                )}
                <p className={`text-xs font-bold uppercase tracking-[0.18em] ${plan.recommended ? "text-white/60" : "text-slate-500"}`}>
                  {plan.number}
                </p>
                <h3 className={`mt-3 font-serif text-3xl ${plan.recommended ? "text-white" : "text-[#132a22]"}`}>
                  {plan.name}
                </h3>
                <div className={`mt-7 mb-8 font-serif text-5xl ${plan.recommended ? "text-white" : "text-[#132a22]"}`}>
                  {plan.price} <span className={`font-sans text-base font-light ${plan.recommended ? "text-white/50" : "text-slate-400"}`}>/ mes</span>
                </div>
                <ul className={`space-y-4 text-sm leading-relaxed ${plan.recommended ? "text-white/80" : "text-slate-700"}`}>
                  {plan.items.map((item) => (
                    <li key={item} className="flex gap-3">
                      <span className="mt-1 text-[#c1a162]">•</span>
                      <span>{item}</span>
                    </li>
                  ))}
                </ul>
          ...
          <div className="mx-auto mt-10 max-w-4xl border border-slate-200 border-l-4 border-l-[#c1a162] bg-slate-50 p-7 text-center">
            ...
            <p className="mt-3 text-sm font-light leading-relaxed text-slate-600">
              La gestión de campañas Meta Ads está incluida en los planes correspondientes. El presupuesto destinado a publicidad se mantiene independiente y es cubierto directamente por Muebles Ideal mediante su propia tarjeta de crédito o débito.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="px-6 py-24 md:px-12 md:py-32">
        <div className="mx-auto max-w-6xl">
          <div className="max-w-4xl">
            ...
            <p className="mt-10 border-l-4 border-[#132a22] pl-6 text-xl font-light leading-relaxed text-slate-600 sm:text-2xl">
              “Que Muebles Ideal no solamente publique productos, sino que construya una presencia digital capaz de transmitir la calidad de sus muebles antes de que el cliente llegue al showroom.”
            </p>
          </div>
          ...
          <div className="mt-24 grid gap-10 md:grid-cols-3">
            {[
              ...
              <div key={title} className="border-t border-[#c1a162] pt-6">
                <h3 className="font-serif text-2xl sm:text-3xl">{title}</h3>
                <p className="mt-4 text-base font-light leading-relaxed text-slate-600">{text}</p>
              </div>
            ))}
          </div>
          ...
          <div className="mt-20 bg-[#132a22] p-10 text-center text-[#f9f8f6] sm:p-16 lg:p-20">
            <p className="font-serif text-3xl leading-tight sm:text-5xl">
              ...
              <br />
              <span className="italic text-[#c1a162]">Publicidad que conecta.</span>
              <br />
              Una marca con 50 años que sigue evolucionando.
            </p>
          </div>
          ...
          <div className="mt-12 border-t border-slate-200 pt-8">
            ...
            <div className="mt-6 grid gap-4 text-sm font-light leading-relaxed text-slate-500 md:grid-cols-2 lg:grid-cols-3">
              ...
              <p className="font-medium text-[#132a22] lg:col-span-2">• La inversión publicitaria de Meta Ads no está incluida en los valores de los planes.</p>
            </div>
          </div>
