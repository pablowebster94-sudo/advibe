        ...
        <div className="mt-20 border-y border-white/10">
          {demos.map((item, index) => (
            <button key={item.id} onClick={() => setActive(index)} aria-pressed={index === active} className={`group relative grid w-full gap-4 border-b border-white/10 py-7 text-left last:border-b-0 sm:grid-cols-[90px_1fr_auto] sm:items-center sm:py-8 ${index === active ? "bg-white/[0.025]" : "hover:bg-white/[0.015]"}`}>
              <span className={`font-mono text-xs tracking-[0.22em] ${index === active ? "text-lime-300" : "text-slate-400"}`}>{item.index}</span>
              <span className={`text-2xl font-semibold tracking-[-0.035em] transition-colors duration-200 sm:text-3xl ${index === active ? "text-white" : "text-slate-400"}`}>{item.label}</span>
              <span className={`text-lg transition-colors duration-200 ${index === active ? "text-lime-300" : "text-white/15 group-hover:text-white/50"}`} aria-hidden="true">↗</span>
              {index === active && <span className="absolute bottom-0 left-0 h-px w-full bg-lime-300" aria-hidden="true" />}
            </button>
          ))}
        </div>

        <div className="mt-10 grid overflow-hidden border border-white/10 lg:grid-cols-[1.25fr_0.75fr]">
          <div className="p-7 sm:p-10 lg:p-12">
            ...
            <div className="mt-10 grid gap-8 border-t border-white/10 pt-8 sm:grid-cols-2">
              <div><p className="text-[10px] uppercase tracking-[0.28em] text-slate-400">Antes</p><p className="mt-3 text-sm leading-6 text-slate-500">{demo.before}</p></div>
            ...
            <div className="mt-8 border-t border-white/10 pt-8"><p className="text-[10px] uppercase tracking-[0.28em] text-slate-400">Cómo funciona</p><p className="mt-3 max-w-2xl text-sm leading-7 text-slate-400">{demo.insight}</p></div>
          </div>
          ...
          <div className="flex flex-col justify-between border-t border-white/10 bg-white/[0.02] p-7 sm:p-10 lg:border-l lg:border-t-0 lg:p-12">
            <div><p className="text-[10px] uppercase tracking-[0.3em] text-slate-400">Enfoque AdVibe</p><p className="mt-6 text-6xl font-semibold tracking-[-0.07em] text-lime-300 sm:text-7xl">{demo.metric}</p><p className="mt-2 text-sm text-slate-500">{demo.metricLabel}</p></div>
            <div className="mt-12 border-t border-white/10 pt-6"><p className="text-sm font-semibold text-white">Cada sistema se diseña a medida.</p><p className="mt-2 text-xs leading-5 text-slate-400">No añadimos tecnología por moda. La usamos cuando resuelve un problema real.</p></div>
